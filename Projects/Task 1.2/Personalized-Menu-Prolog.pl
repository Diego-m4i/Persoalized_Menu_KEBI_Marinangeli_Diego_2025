...
% Ingredient types
type(vegetable_and_plant-based).
type(dairy).
type(meat).
type(fish).
type(cereal_and_grains).
...
% For a light (diet friendly) profile: the dish must be between 100 and 300 kcal.
check_calories(light, Dish) :-
    dish_calories(Dish, Total),
    Total >= 100,
    Total =< 300.

% For a moderate (balanced) profile: the dish must be between 300 and 500 kcal.
check_calories(balanced, Dish) :-
    dish_calories(Dish, Total),
    Total > 300,
    Total =< 500.

% For a hearty (energy rich) profile: the dish must be between 500 and 1000 kcal.
check_calories(rich, Dish) :-
    dish_calories(Dish, Total),
    Total > 500,
    Total =< 1000.
...
suggest_menu("Michela", Suggest) :- recommended_dish(_, light, _, Suggest).
suggest_menu("Mattia", Suggest) :- recommended_dish(vegetarian, hearty, [eggs, gluten], Suggest).
suggest_menu("Francesco", Suggest) :- recommended_dish(carnivor, high, _, Suggest).
suggest_menu("Diego", Suggest) :- recommended_dish(vegan, moderate, [lactose], Suggest).
